...
编辑器中的“日志目录（可选）”仅用于后台应用。留空时使用上述任务专用目录内的 `logs`；也可以填写或浏览选择一个绝对路径作为自定义日志目录。主窗口选择后台任务后点击“打开任务目录”，程序会打开该任务的专用运行目录（内含 `wrapper.ps1`、`config.json` 与 `logs` 子目录）。
...
后台任务自己的 stdout/stderr 不写入主应用日志，而是以目标程序输出的原始字节写入默认或用户指定的任务日志目录。每次启动会重新创建 `stdout.log` 和 `stderr.log`。可在主窗口选择后台任务后点击“打开任务目录”打开该任务的运行目录，再进入 `logs` 子目录查看日志。
